...
fig1 = plt.figure(1)
ax = plt.axes(projection='3d', title= 'Espaço de trabalho', xlabel= 'x [mm]', ylabel= 'y [mm]', zlabel= 'z [mm]')
#ax.view_init(18, 142)
ax.view_init(200,80) #160,45
...
for phi1 in np.arange (-90, 90, 5, dtype=int):
    for theta2 in np.arange(-45, 135, 5, dtype=int):
        for theta3 in np.arange(0, 180, 5, dtype=int):
            ...
            x = np.cos(phi1_rad)*(l2*(np.cos(theta2_rad+theta3_rad))+l1*np.cos(theta2_rad))
            y = np.sin(phi1_rad)*(l2*(np.cos(theta2_rad+theta3_rad))+l1*np.cos(theta2_rad))
            z = l2*np.sin(theta2_rad+theta3_rad)+l1*np.sin(theta2_rad)
            height.append(z)
...
p0 = [0, 0, 0]
p1 = [l1*np.cos(theta2_l), l1*np.cos(theta2_l)*np.tan(phi1_l), l1*np.sin(theta2_l)]
p2 = [np.cos(phi1_l)*(l2*(np.cos(theta2_l+theta3_l))+l1*np.cos(theta2_l)),
      np.sin(phi1_l)*(l2*(np.cos(theta2_l+theta3_l))+l1*np.cos(theta2_l)),
      l2*np.sin(theta2_l+theta3_l)+l1*np.sin(theta2_l)]
