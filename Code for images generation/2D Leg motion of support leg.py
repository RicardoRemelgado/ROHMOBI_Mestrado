...
for t in np.arange (0, 1, 0.01, dtype=float):
    plt.figure(t*100)
    ax = plt.axes(title='Trajectory of leg in support phase', xlabel='X axis (mm)', ylabel='Z axis (mm)')
    ax.set_xlim(0, 100)
    ax.set_ylim(0, -100)
    ax.invert_yaxis()
    ...
    #Save the fgure
    plt.savefig(r'C:\Universidade\10º Semestre - Tese de mestrado\Python code\2D Images of support leg motion\ ' + str(int(t*100)) + ".jpg")
    plt.close()
