...
//Define global variables
  ...
  float P1stswing [4][3] = {{0, 0, -60},
                           {0, 0, -45},
                           {50.9, 0, -58.1},
                           {36, 0, -60}};
//list of control points of Bezier Curve in the first swing
  float P1stsup [3][3] = {{0, 0, -60},
                         {-18, 0, -65},
                         {-36, 0, -60}};
...
int fPos[] = {-6+5, 10+5, 3+5, -9+5, 14-5, -3+5, -3, 9, -3, 12, 8, 9, 7, -5, -2}; //correct the different heights of the modules 
  //Mover of phases
...
//Initial stance
  ...
  writeServos(12, DegServo_p1(0 + fPos[12]));
    //Leg1
    writeServos(0, DegServo_t2(27.45 + fPos[0]));
    writeServos(1, 113.58+ fPos[1]);
    //Leg2
    writeServos(2, ConvertLeg2(DegServo_t2(27.45 + fPos[2])));
    writeServos(3, ConvertLeg2(113.58 + fPos[3]));
  //2nd module
  writeServos(13, DegServo_p1(0 + fPos[13]));
    //Leg1
    writeServos(4, DegServo_t2(27.45 + fPos[4]));
    writeServos(5, 113.58 + fPos[5]);
    //Leg2
    writeServos(6, ConvertLeg2(DegServo_t2(27.45 + fPos[6])));
    writeServos(7, ConvertLeg2(113.58 + fPos[7]));
  //3rd module
  writeServos(14, DegServo_p1(0 + fPos[14]));
    //Leg1
    writeServos(8, DegServo_t2(27.45 + fPos[8]));
    writeServos(9, 113.58 + fPos[9]);
    //Leg2
    writeServos(10, ConvertLeg2(DegServo_t2(27.45 + fPos[10])));
    writeServos(11, ConvertLeg2(113.58 + fPos[11]));
...
void ControlPoints(float angle){
  float A[4][3] = {{(0*(angle/3)), 66.22, 100.43},
            {(1*(angle/3)), 15.71, 131.79},
            {(2*(angle/3)), 0, 90},
            {(3*(angle/3)), 4.29, 100.43}};
  for (int i = 0; i<4; i++){
